...
import { revalidateLongEvents } from '@/lib/cache/revalidate';
...
export const LongEventForm = ({ onSuccess, initialData, eventId }: LongEventFormProps) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Revalidate events cache after long event modification (don't throw on failure)
      if (session?.access_token) {
        try {
          await revalidateLongEvents(session.access_token);
